...
module.exports = function(server) {
        ...
        config: {
            validate: {
                ...
                payload: Joi.object().keys({
                    ...
                    dimensions: slideDimensions,
                    transition: slideTransition,
                    annotations: Joi.array().items(Joi.object().keys({
                        uri: Joi.string().allow(''),
                        type: Joi.array().items(Joi.string()).optional(),
                        name: Joi.string().allow(''),
                        inlineId: Joi.string().allow(''),
    ...
    server.route({
        method: 'GET',
        path: '/slide/{id}/annotations',
        handler: handlers.getSlideAnnotations,
        config: {
            validate: {
                params: {
                    id: Joi.string().description('Identifier of slide in the form {id}-{revision}'),
            ...
            tags: ['api'],
            description: 'Get the annotations for a slide',
        },
    });
